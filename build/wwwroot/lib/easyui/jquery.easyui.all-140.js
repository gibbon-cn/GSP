...
(function($){
	...
	function setSize(target, param){
		...
		if (!isNaN(parseInt(opts.height))){
			...
			var distance = pheader._outerHeight() + panel._outerHeight() - panel.height();
	...
	$.fn.panel.methods = {
		...
		doLayout: function(jq, all){
			return jq.each(function(){
				...
				var s = $(this).find('div.panel:visible,div.accordion:visible,div.tabs-container:visible,div.layout:visible,.easyui-fluid:visible').filter(function(index, el){
...
(function($){
	...
	function setContentSize(target, param){
		var t = $(target);
		var noheader = t.dialog('options').noheader;
...
//			bindEvents(this);
	...
	$.fn.validatebox.defaults = {
		...
		events: {
			...
			click: function(e){$(e.data.target).trigger('focus');}
		},
...
(function($){
	function init(target){
		...
		span.bind('_resize', function(e, force){
			if ($(this).hasClass('easyui-fluid') || force){
				setSize(target);
			}
			return false;
...
(function($){
	...
	function buildFileBox(target){
		...
		$(target).addClass('filebox-f').textbox($.extend({}, opts, {
			onClickButton:function(){
				state.filebox.find('.textbox-value').click();
				opts.onClickButton.call(target);
			}
		}));
		...
		file.attr('name', $(target).attr('textboxName')||'').change(function(){
...
(function($){
	function buildSpinner(target){
		...
		arrowIcon.append('<a href="javascript:void(0)" class="spinner-arrow-up"></a>');
		arrowIcon.append('<a href="javascript:void(0)" class="spinner-arrow-down"></a>');
...
(function($){
    ...
    var plugins = [
        'combo','combobox2','combobox','combotree','combogrid','datebox','datetimebox',
        'datetimespinner','numberbox','numberspinner','timespinner','filebox',
        'searchbox','textbox','spinner','lookupbox','adplookupbox','adpcombogrid'
    ];
    ...
    if($.fn.my97datebox){
        $.fn.my97datebox.defaults.height = 22;
    }
    ...
    var linkbutton = $.fn.linkbutton;
    $.fn.linkbutton = function(options,param){
        if(typeof options != 'string' ){
            return this.each(function() {
                var self = this;
                linkbutton.call($(this), options, param);

                $(self).unbind('click.linkbutton');
            });
        } else {
			return linkbutton.call(this, options, param);
		}
    };

    $.fn.linkbutton.methods = linkbutton.methods;
    $.fn.linkbutton.defaults = linkbutton.defaults;
    $.fn.linkbutton.parseOptions = linkbutton.parseOptions;

    $.extend($.fn.linkbutton.methods,{
        setText:function(jq,text){
            if(text =='') {
                $(jq).find('.l-btn-text').addClass('l-btn-empty').html('&nbsp;');
            }
            else{
                $(jq).find('.l-btn-text').removeClass('l-btn-empty').html(text);
            }
        },
        getText: function(jq){
            return $(jq).find('.l-btn-text').text();
        }
    });

    $.fn.pagination.defaults.layout=['list','sep','first','prev','sep','manual','sep','next','last','sep'];

/*
    var pagination =$.fn.pagination;

    //pagination.defaults.paginationAlgin = "left";
    pagination.defaults.layout=
         ['list','sep','first','prev','next','last','sep','manual','sep'];

    $.fn.pagination = function(options, param){
        if (typeof options != 'string'){
            return this.each(function(){
                var self = this;
                pagination.call($(this), options, param);
                // 移动displayMsg 的位置到页码区域的前边
                var paginationOptions = $(this).pagination('options');

                // 总记录信息
                var paginationInfo = $(this).find('.pagination-info').css('marginLeft',0);
                var pagecountCell = $(this).find('.pagination-num').parent().next();


                var td = $('<td><span style="font-size: 13px;line-height: 32px">页</span></td><td></td><td><div class="pagination-btn-separator"></div></td>').insertBefore(pagecountCell);

                var gobtn = $('<a style="height:30px"></a>').linkbutton({ //
                    iconCls:'pagination-jump',plain:true
                }).on('click',function(){
                    var pagenumber = $(this).find('.pagination-num').val() || 1;
                    $(self).pagination('select',pagenumber);
                });

                td.eq(1).append(gobtn);



                pagecountCell.find('span').css({
                    float:'left',
                    lineHeight:'32px'
                });
                //var sep = $('<div class="pagination-btn-separator"></div>').appendTo(pagecountCell);
                paginationInfo.insertAfter(pagecountCell);

//                if(paginationOptions.paginationAlgin == "right") {
//                    var paginationInfo = $(this).find('.pagination-info').css('float', 'right');
//                    var paginationNumber = $(this).find('table').css('float', 'right');
//                    paginationInfo.insertBefore(paginationNumber);
//                }
            });
        } else {
            return pagination.call(this, options, param);
        }
    };
    $.fn.pagination.methods = pagination.methods;
    $.fn.pagination.defaults = pagination.defaults;
    $.fn.pagination.parseOptions = pagination.parseOptions;
...
*/
    ...
    $.extend($.fn.dialog.defaults,{
        onLoad:function(){
            ...
            if(opts.buttons && opts.buttons.length){
                var btns = $(self).dialog('buttons');
                $.each(opts.buttons,function(i,n){
                    if(n.btnCls){
                        $(btns[i]).addClass(n.btnCls);
                    }
                })
            }
        }
    ...
    $.extend($.fn.dialog.methods,{
        buttons: function(jq,index){
            var dlg = $(jq).dialog('dialog');
            var btnContainer = dlg.find('.dialog-button');
            if(!index) {
                return btnContainer.find('a');
            }
            else{
                return btnContainer.find('a').eq(index);
            }
        }
    });
    ...
    $.extend($.fn.datagrid.defaults.editors.text, {
        resize: function (target, width) {
            $(target)._outerWidth(width)._outerHeight(28);
        }
    });

    $.extend($.fn.datagrid.defaults.editors.numberbox, {
        resize: function (target, width) {
            $(target)._outerWidth(width)._outerHeight(28);
            $(target).next('span')._outerHeight(28);
        }
    });
    ...
    $.extend($.fn.datagrid.defaults.editors.datebox, {
        resize: function (target, width) {
            ...
            $(target).next('span')._outerHeight(28).find('.combo-arrow').height(28);
        }
    });
