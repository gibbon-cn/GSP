...
if(opts.position == 'bottom'){ // 2016
    $(_1e4).tooltip("arrow").css('left','10px');
}
...
top=opts.trackMouseY+opts.deltaY;
    ...
    var t = $(_1e4);
    if (( t.parent().is('span') || t.parent().parent().is('span'))
        && opts.position == "left") {
        left = t.offset().left
    } else {
        if (t.parent().is('.fluidWrap')) {
            left = t.offset().left
        } else {
    ...
    switch (opts.position) {
        ...
        case "left":
            ...
            break;
        case "top":
            left = t.offset().left; //(tip._outerWidth()-t._outerWidth())/2;
...
var s=$(_250).find("div.panel,div.accordion,div.tabs-container,div.layout,.easyui-fluid").filter(function(_252,el){
if($(el).is(':visible')) { //2015
...
t.trigger("focus");
    ...
    if($(this).data('tooltip')) { // 20170519
        var tip = $(this).tooltip("tip");
        if (tip) {  // 2016
            tip.css({color: "#000", borderColor: "#CC9933", backgroundColor: "#FFFFCC"});
        }
    }
},onHide:function(){
$(this).tooltip("destroy");
...
// 分组合计行不触发双击事件
if(tr.hasClass('datagrid-group-totalrow')){
        return false;
    }
...
if(!opts.stopEditOnAnyTime) { //
    return _69b.length == 0;
}else{ // 不进行数据验证，就可以结束编辑
    return true;
}
};
...
var _6c7=opts.loader.call(_6c3,_6c5,function(data){
    if($(_6c3).data('datagrid') == null){
        return false;// 2016/09/07 在低版本浏览器中，帮助窗口还没有加载完数据时，就关闭窗口报错。
    }
setTimeout(function(){
...
if(!$(this).data('datagrid')){ //2016/09/07 控件未初始完毕就被移除
    return false;
}
...
if ($.fn.combo){
	$.fn.combo.defaults.missingMessage = '该输入项为必填项';
}
...
if ($.fn.adpcombogrid){
	$.fn.adpcombogrid.defaults.missingMessage = '该输入项为必填项';
}
if ($.fn.adplookupbox){
	$.fn.adplookupbox.defaults.missingMessage = '该输入项为必填项';
}

if ($.fn.timespinner){
	$.fn.timespinner.defaults.missingMessage = '该输入项为必填项';
}
